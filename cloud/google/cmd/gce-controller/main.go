...
func main() {
	...
	var controllerType, kubeadmToken, machineSetupConfigsPath string
	fs.StringVar(&controllerType, "controller", controllerType, "specify whether this should run the machine or cluster controller")
	fs.StringVar(&kubeadmToken, "token", kubeadmToken, "Kubeadm token to use to join new machines")
	...
	if controllerType == "machine" {
		machineServer := machineoptions.NewMachineControllerServer(kubeadmToken, machineSetupConfigsPath)
